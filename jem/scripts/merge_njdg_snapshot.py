...
def plan_report(snapshot_path: Path, out_path: Path) -> None:
    ...
    lines.append(
        "\nGenerated by `merge_njdg_snapshot.py --plan`. "
        "Re-running the script overwrites this file.\n\n"
    )
    lines.append(f"- Snapshot: `{snapshot_path}`\n")
    lines.append(f"- Entities with `_detail.case_volume` in the snapshot: {len(cv_map)}\n")
    lines.append("\n## Per entity\n")
    ...
    for sid in sorted(cv_map.keys()):
        ...
        if yml:
            ...
            lines.append(f"| `{sid}` | `{tid}` | none | no YAML, skip |\n")
