...
Copy everything inside the prompt box below into Claude, Cursor Agent, or ChatGPT, with the JEM repo open as context. It is one prompt for both co-maintainers and contributors. Your `ROLE` and `TASK` lines control what the assistant may do.

Phased build tasks (on the way to 1,500+ entities): pick a `TASK` from [`ENTITY_BUILD_ROADMAP.md`](ENTITY_BUILD_ROADMAP.md). Use only the prompts listed under Active prompts, since finished categories are archived there.
...
|------|-------------|---------------------|
| Contributor | Use the prompt plus a roadmap `TASK`, produce YAML drafts, then attach them to a [GitHub issue](https://github.com/datastiltskin/jem/issues/new?template=data_correction.yml). Issues only. | `validate.py`, merge, relationships, release |
| Co-maintainer | Set `ROLE: co-maintainer`, validate locally, open a PR | Review issues, merge, wire relationships |

| Rule | Detail |
|------|--------|
| Submissions | GitHub issues only, no email |
| New entities | Contributors may submit proposed YAML, and maintainers merge after validation |
...
```
You are helping with Judiciary Entity Map (India), or JEM, an open CC0 structural dataset of Indian courts, tribunals, regulators, and related bodies. You are working in a clone of the JEM repository.
...
• Entity IDs: permanent snake_case, such as hc_madras or mh_district_court_pune. Never rename an existing id.
...
CATEGORY_ID: [such as C15 UP pack, used for the issue title]
ENTITY_ID (if editing existing): [id or none]
STATE / CLUSTER (if relevant): [such as KL, regulatory_bodies, backbone]
...
2. Produce ONLY entity YAML file(s) as deliverables, with sources[] on every factual claim you add or change.
...
1. Save the YAML from the chat.
2. Open a [GitHub issue](https://github.com/datastiltskin/jem/issues/new?template=data_correction.yml), and nothing else. Title it something like `[data] C15 UP pack`, attach the files, and include the field/source table.
3. Maintainers validate with `python3 scripts/validate.py --entity path/to/file.yaml`
...
When a category is complete, set its status to done in `ENTITY_BUILD_ROADMAP.md`, archive the prompt, and sync [README entity build progress](../../README.md#entity-build-progress).
