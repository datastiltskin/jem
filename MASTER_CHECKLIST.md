# Judiciary Entity Map (India) - JEM: master build and deployment checklist
# Generated: April 25 2026
# Last full repo audit: Jun 29 2026, after the QA sprint and people/roles layer. See PROGRESS & REPO AUDIT below.
...
# - Never edit `jem/data/derived/` by hand, and never hand-edit `graph.json`. Rebuild via the scripts.
...
## REPO AUDIT (Jun 29 2026, after the QA sprint)
...
Recent work (Jun 15, Wave 4 and 5 plus v1.2): C15 to C18 state packs, meaning core entities for all remaining states and UTs (AP, TS, GJ, MP, BR, KL, PB, HR, OD, NE states, CG, GA, HP, JH, UK, CH, AN, LD, LA, JK, SK, and PY expanded). Per-state relationship packs, meaning `{st}_relationships.yaml` for all 35 state folders plus `c26_backbone_orphan_relationships.yaml` and `c26_generic_orphan_relationships.yaml`, wiring 22 generic orphans. Entity count went from 668 to 1,103, relationships from 748 to 1,855, and orphans from 175 to 0.

Prior (Jun 12, Phase 1 batch): M-T1 ITAT×25, M-T2 to M-T6, C15 UP/WB/RJ, and C10/C11. Entities went from 599 to 668.

Prior (May 20): the central tribunal, regulator, and ministry batch, the IIAC rename, and governance graph Phase 6. Operator docs: [`jem/docs/SESSION_WORKFLOW.md`](jem/docs/SESSION_WORKFLOW.md) · [`jem/docs/ENTITY_BUILD_ROADMAP.md`](jem/docs/ENTITY_BUILD_ROADMAP.md).
...
- [x] **NJDG snapshot merge (state / HC / tribunal rollups):** `merge_njdg_snapshot.py --apply` on 139 entities. See Part 3.5.
...
- [x] **Governance score exclusions:** in `derive.py` and [`jem/docs/DATA_MODEL.md`](jem/docs/DATA_MODEL.md). PM, ministers, and ministries are excluded from IR/DP, and `AppointmentBody` committees are still scored.
...
- [x] **C20 labour (partial):** CGIT×6, `cgit_principal`, EPFAT, and EPFO, with BenchOf and MoLE funding/oversight wired (`cgit_epfat_mole_relationships.yaml`). Remaining: ESI courts and the state labour court generic.
- [x] **Tribunal completion (Jun 12):** DRAT BenchOf×5, CESTAT→HC×9, tax appellate edges in `tribunal_completion_jun2026.yaml`.
- [x] **HC numerics pass (Jun 2):** `judge_strength` on constitutional and HC entities, 40/668 populated.
...
Critical path to structural completeness (~1,500): P0 → P1 → P3 → P4. Critical path to metric usefulness: P2 plus P5. Critical path to product completeness: P6.

### Coverage-gap thread: v1.0.0 deploy still pending, and post-May growth moves to the VERSION ROADMAP

> Deploy decision (May 20, still valid): ship v1.0.0 once the operator runs the runbook and `validate.py` reports 0 errors. The corpus has since grown from 506 to 1,103 entities pre-deploy. It is acceptable to tag v1.0.0 at the current head or cut from an earlier commit, with the remainder scheduled for v1.1 and later.
...
| KA Dharwad district→bench verify | Deferred | v1.1 |
| Relationship orphans (~175), incremental wiring | Done (0 remain, via the C26 packs) | v1.1 |
| §7 `judge_strength` bulk fill | Partial (39/1,103) | v1.2 |
| §3 `case_volume` bulk fill | Partial (84/1,103) | v1.2 |
| C04 DRT remaining benches (14 of 39) | Deferred (gated on drt.gov.in) | v1.5 / Phase 3 |
| C20 labour court wiring (ESI, state labour courts) | Partial (scaffolds in graph; CGIT/EPFAT/EPFO wired) | v1.5 |
| Per-district NJDG (TN 37/38, MH/KA bootstrap) | Parked | v1.3 |
| §5 full state district lattices (core packs done, TN/MH/KA deep) | Core packs done, per-district expansion deferred | v1.4 |
| Gap registry entities (tax/labour/defence benches) | Mostly done: GSTAT benches gated | v1.5 (Part 5) |

### v1.0.0 release (operator step, tagged Jun 2026, production deploy pending)
...
### Parked (scheduled later, and does not block the v1.0.0 tag)

- [ ] **District-level NJDG exports**: v1.3 (Part 3.5.2)

### Deferred to v2.0 (product and infra, not v1.x data releases)

> Naming: v1.x means data and graph semver tags on production deploy. v2.0 means UI, live API, and GitHub (Part 4), which is separate from v1.1 to v1.5 below.
...
- [ ] v1.1: KA Dharwad verify plus generic `state_data` (orphans are clear)
...
Semantic data releases after the v1.0.0 deploy. Tag with `git tag -a v1.x.y` after validate, build, and smoke tests on the production URL.

| Release | Scope | Acceptance |
|---------|--------|------------|
| v1.0.0 | Ship graph at deploy time (1,145 entities, 1,810 rels; UI merge Jun 16). | `validate.py` 0 errors, local tag done, friedso deploy done (Jun 2026) |
| v1.1 | **Structural integrity**: no new external datasets required | Config matches graph, UP/WB/RJ bench edges done, orphans at 0, and KA Dharwad plus generic `state_data` remain |
...
- [x] Tag `v1.0.0` (Jun 16 2026, UI merge plus full corpus on `main`)
...
> Do not start until district-level NJDG exports exist. The state and HC rollup merge is complete in v1.0.0.
...
- [x] §5.1 Tax/revenue stack, partial. Done: CESTAT×8, CIT(A)/JCIT(A)/AO generics, AAR/CAAR, `gstat_bench_generic`. Remaining: GSTAT constituted benches, VAT tribunal per state.
- [x] §5.2 Labour, partial. Done: CGIT×6 plus principal wired (MoLE funding), EPFAT and EPFO in graph, `state_labour_court_generic` and `esi_court_generic` scaffolds. Remaining: ESI and labour court relationship wiring.
...
- [x] Run validate.py: confirm clean *(May 19 2026, 0 errors across 494+ entity files)*
...
- [x] **Bookmark:** [`jem/docs/SESSION_WORKFLOW.md`](jem/docs/SESSION_WORKFLOW.md), covering validate → derive → build, graph and bundle overwrite risks, `build_safe.sh`, and the deploy pointer
...
### 3.1 Maharashtra (MH): ~51 entity YAML files, done
...
### 3.2 Delhi (DL): ~22 entity YAML files, done
...
### 3.3 Karnataka (KA): ~43 entity YAML files, done
...
### 3.3a Tamil Nadu (TN) and Puducherry (PY): done
...
### 3.5.2 Parked: district-level NJDG exports, scheduled for v1.3

> Do not start until district exports exist. The state and HC rollup merge shipped in v1.0.0, and per-district work is v1.3.
...
| TN collapsed generic | 1 | `tn_district_courts_generic` (state rollup), done | Refresh when new state snapshot available |
...
**Status (May 19 2026):** partial. The data model and map UX advances are below, and the full Canvas, Sankey, and journey spec is still outstanding.
...
- [ ] `filing_cost_range`, `appeal_cost_range` added to schema (left empty as a community task)
...
### 5.6 State level, all states and UTs (core packs done Jun 15, deep lattices in v1.4)
...
|-----------|-------------------------------|----------------------------------------|
| Bulk YAML from templates | Good: 10 to 50 entities per session | Poor: token-heavy, and drifts on repetition |
| `validate.py` / `derive.py` / `build.py` loop | Good: native terminal, fixes on failure | Mixed: possible but awkward |
| Generator scripts (`generate_v1_states_bundle.py`, lattice bootstrap) | Good: edit, run, and diff in one session | Mixed: better for design review than execution |
| Relationship wiring (maintainer) | Good: mechanical edges from config tables | Good: better when appellate routing is ambiguous |
| Schema / `derive.py` formula changes | Mixed: can do it, but easy to miss knock-on effects | Good: reads the full `entity_schema.yaml` and score logic |
| Independence-risk / structural-gap reasoning | Mixed: shallow without sources | Good: statute and collegium context |
| V2 architecture (Canvas, Sankey, `fetch_njdg.py`) | Good: implementation once a spec exists | Good: system design and API shape |
| Primary-source research (gazettes, DoJ reports) | Mixed: needs URLs supplied in the prompt | Good: synthesizes across sources |
| GitHub issues / contributor YAML triage | Good: validate and merge | Mixed: overkill |
| Essay / Pendency Lab / external writing | Poor: wrong tool | Good: keep it separate, with no code context |
...
Never parallelize two agents editing the same `{st}_relationships.yaml`, or two `graph.json` builds on one branch.
...
Rule of thumb: if a task needs domain reasoning about Indian judicial structure, or new code architecture, use Claude. If it needs more YAML written from a known template, use Cursor.
...
| V1 + TN + PY | ~157 | ~946 KB | May 2026, done |
| V1 + MH + DL + KA + full corpus | 494 | ~1.87 MB | May 19 2026, done |
| NJDG snapshot merge (rollup) | 494 | ~1.87 MB | May 19 2026, done |
| TN generic + 38-district lattice | 494 | ~1.87 MB | May 19 2026, done |
| Central tribunal + ministry batch | 506 | ~1.86 MB | May 20 2026, done |
| Batch 3 + C04/C19/C20 partial | 592 | ~2.94 MB | Jun 12 2026, done |
| Tribunal completion + drt_b5 (C20 MoLE wiring) | 599 | ~3.02 MB | Jun 12 2026, done |
| Phase 1 batch (ITAT×25, UP/WB/RJ, C10/C11, M-T1-M-T6) | 668 | ~3.34 MB | Jun 12 2026, done |
| Wave 4-5 (all state/UT core packs + rel wiring) | 1,103 | ~5.63 MB | Jun 15 2026, done |
| v1.0.0 tag + production deploy | 1,145 | ~5.8 MB | Tagged Jun 16 2026 · friedso deploy done (Jun 2026) |
