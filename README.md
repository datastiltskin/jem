...
|---|---:|---:|---:|
| **institution** | 403 | 436 | 226 |
| **personnel** | 8 | 0 | 14 |

That is 1,087 countable entities. A further 64 entities carry `is_generic_rollup: true` — placeholders such as `lok_adalat_generic` and the per-state `*_district_courts_generic` scaffolds that stand in for a class of body rather than naming one. They render on the map and they anchor edges, but they are not institutions you could visit, so counting them would overstate the corpus. 1,087 + 64 = the 1,151 files on disk.

Two numbers in `graph.json` metadata look like they disagree and do not. `meta.entity_count` is 1,151, every node the renderer draws, rollups included. `meta.entity_counts.total_countable` is 1,087, the corpus size for reporting. They answer different questions and are named apart so that neither gets quoted as the other.
...
1,087 countable entities of roughly 1,500 structural entities are in the repo (v1.0.0, Jun 2026), held in 1,151 files alongside 64 generic rollups. Full phased prompts and the maintainer workflow live in [`jem/docs/ENTITY_BUILD_ROADMAP.md`](jem/docs/ENTITY_BUILD_ROADMAP.md).
...
| C27 | Data-quality upgrades | 1151 | updated | all |
| C28 | NJDG / judge_strength numerics | 1151 | partial (see below) | 2 |
...
| Chunked graph.json | Required at 1500+ entities. ~5.9 MB is manageable at 1,151. |
